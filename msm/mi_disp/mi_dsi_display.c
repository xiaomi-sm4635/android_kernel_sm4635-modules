			...
			char *buf, size_t size)
{
	...
	if (!strlen(wp_info_str)) {
		...
		for (i = 0; i < panel->wp_config.wp_cmds_rlen; i++) {
			snprintf(wp_info_str + i * 2, size - (i * 2), "%02x", panel->wp_config.return_buf[i]);
		}
	}

	DISP_TIME_INFO("%s display wp info is %s\n", dsi_display->display_type, wp_info_str);
	ret = snprintf(buf, size, "%s\n", wp_info_str);
	return ret;
}
		...
		char *buf, size_t size)
{
	...
	if (mi_get_panel_id_by_dsi_panel(display->panel) == PANEL_ID_MAX) {
		rdlen = 13;
		rc = dsi_display_cmd_receive(display, cmd, sizeof(cmd), rdbuf, rdlen, &ts);
		if (rc == rdlen) {
