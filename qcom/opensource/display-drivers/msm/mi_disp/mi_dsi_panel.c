...
int mi_dsi_update_switch_cmd(struct dsi_panel *panel)
{
	...
	if (panel->mi_cfg.feature_val[DISP_FEATURE_FLAT_MODE] == FEATURE_ON) {
		...
		lhbm_gamma_cfg = panel->cur_mode->priv_info->flat_on_lhbm_gamma;
	} else {
		gamma_cfg = panel->cur_mode->priv_info->flat_off_gamma;
		lhbm_gamma_cfg = panel->cur_mode->priv_info->flat_off_lhbm_gamma;
	}
		...
		if (cmd->msg.tx_len >= 3) {
			...
			if (mi_get_panel_id_by_dsi_panel(panel) == M2_PANEL_PA) {
				if (mi_cfg->feature_val[DISP_FEATURE_HBM] == FEATURE_ON) {
					bl_lvl = bl_lvl + panel->bl_config.bl_max_level;
				}
			}
			...
			u32 doze_brightness)
{
	...
	DISP_ERROR("display: mi_dsi_panel_set_doze_brightness enter \n"); 
			...
			u32 *doze_brightness)
{
	...
	DISP_ERROR("display: mi_dsi_panel_get_doze_brightness \n");  
...
#else
		if (mi_get_panel_id_by_dsi_panel(panel) == M2_PANEL_PA ||
			mi_get_panel_id_by_dsi_panel(panel) == M3_PANLE_PA) {
			if (ctl->feature_val == FEATURE_ON) {
...
#endif
		mi_dsi_panel_state_count(panel, PANEL_HBM, ctl->feature_val);
		mi_disp_feature_event_notify_by_type(mi_get_disp_id(panel->type),
			...
			enum dsi_cmd_set_type type, int bl_lvl)
{
	...
	if (bl_lvl <= mi_cfg->panel_1920pwm_dbv_threshold ||
		refresh_rate == 120 || panel->qsync_enable) {
...
int dsi_panel_send_em_cycle_setting(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (!config->build_id || bl_lvl == 0) {
		...
		if (bl_lvl <= mi_cfg->panel_1920pwm_dbv_threshold && !mi_cfg->is_em_cycle_16_pulse) {
			...
			mi_cfg->is_em_cycle_16_pulse = true;
			if (rc) {
				DISP_ERROR("[%s] failed to send EM_CYCLE_16PULSE_ON cmds, rc=%d\n",
						panel->name, rc);
			} else {
				DSI_INFO("[%s] send EM_CYCLE_16PULSE_ON cmds, 16_pulse: %d\n",
						panel->name, mi_cfg->is_em_cycle_16_pulse);
			}
			mi_disp_update_0size_lhbm_info(panel);
		} else if (bl_lvl > CVC_THRESHOLD && mi_cfg->is_em_cycle_16_pulse) {
			mi_cfg->is_em_cycle_16_pulse = false;
			mi_disp_update_0size_lhbm_info(panel);
			...
			if (rc) {
				DISP_ERROR("[%s] failed to send EM_CYCLE_16PULSE_OFF cmds, rc=%d\n",
                                                panel->name, rc);
			} else {
				DSI_INFO("[%s] send EM_CYCLE_16PULSE_OFF cmds, 16_pulse: %d\n",
						panel->name, mi_cfg->is_em_cycle_16_pulse);
			}
		}
	}
...
int dsi_panel_parse_wp_reg_read_config(struct dsi_panel *panel)
{
	...
	DISP_ERROR(" zgq add dsi_panel_parse_wp_reg_read_config\n");
...
int mi_dsi_panel_update_mipi_qsync_on_cmd(struct dsi_panel *panel, u32 cmd_update_val)
{
	...
	if (mi_cfg->active_qsync_min_fps != cmd_update_val) {
		...
		rc = mi_dsi_panel_update_cmd_set(panel, panel->cur_mode, type, info, cmd_val_buf, cmd_val_size);
	} else {
		DISP_INFO("Same qsync config:%d, %d, %d\n",mi_cfg->active_qsync_min_fps, cmd_update_val, mi_cfg->active_qsync_min_fps);
		rc = 0;
	}
exit:
	return rc;
