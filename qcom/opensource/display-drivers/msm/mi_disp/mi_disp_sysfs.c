			...
			struct device_attribute *attr, char *buf)
{
	...
	if(NULL!=strstr(lcd_name,"c3f2 43 02 0a video mode hd dsi panel")) {
		whitepoint_x=300+whitepoint_x-0x80;
		whitepoint_y=320+whitepoint_y-0x80;
	} else if(NULL!=strstr(lcd_name,"c3f2 37 02 0b video mode hd dsi panel")) {
		whitepoint_x=300+whitepoint_x-0x80;
		whitepoint_y=320+whitepoint_y-0x80;
	} else {
		// do nothing
		DSI_INFO("no panel compatible, use %d,%d\n",whitepoint_x,whitepoint_y);
	}
